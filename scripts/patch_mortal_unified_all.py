...
def apply(root: Path, *, rust_only: bool = False, python_only: bool = False) -> None:
    ...
    if not python_only:
        ...
        gameplay = (root / "libriichi/src/dataset/gameplay.rs").read_text(encoding="utf-8")
        ...
        if "MORTAL_ROGS_UNIFIED_DATASET_STAGE8B" not in gameplay:
            raise RuntimeError("unified gameplay dataset Stage 8B postcondition failed")
