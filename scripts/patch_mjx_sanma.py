...
def main() -> int:
    ...
    if args.through >= 1:
        apply_stage1(root)
        print("stage 1/3: rule + tile set OK", flush=True)
    if args.through >= 2:
        apply_stage2(root)
        print("stage 2/3: wall + deal OK", flush=True)
    if args.through >= 3:
        apply_stage3(root)
        print("stage 3/3: nuki protocol OK", flush=True)

    post = audit(root, manifest)
    print(json.dumps(post, indent=2, ensure_ascii=False))
    print(
        "MJX_SANMA_PATCH_PARTIAL_OK: stages 1-3 applied; production remains disabled "
        "until state/scoring/legal/parity stages pass.",
        flush=True,
    )
    return 0
